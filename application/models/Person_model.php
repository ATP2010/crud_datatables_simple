	...
	private function _get_datatables_query()
	{
		...
		foreach ($this->columns as $item) // loop column 
		{
			if($_POST['columns']['0']['search']['value'])
			{
				if($i===0){
					$this->db->group_start();
					$this->db->like($item, $_POST['columns']['0']['search']['value']);
				} else {
					$this->db->or_like($item, $_POST['columns']['0']['search']['value']);
				}
			...
			if($_POST['columns']['2']['search']['value'])
			{
				if($i===0){
					$this->db->group_start();
					$this->db->like($item, $_POST['columns']['2']['search']['value']);
				} else {
					$this->db->or_like($item, $_POST['columns']['2']['search']['value']);
				}
			...
			if($_POST['columns']['3']['search']['value'])
			{
				if($i===0){
					$this->db->group_start();
					$this->db->like($item, $_POST['columns']['3']['search']['value']);
				} else {
					$this->db->or_like($item, $_POST['columns']['3']['search']['value']);
				}
				if(count($this->column) -1 == $i)
					$this->db->group_end();
			}
			...
			if($_POST['columns']['4']['search']['value'])
			{
				if($i===0){
					$this->db->group_start();
					$this->db->like($item, $_POST['columns']['4']['search']['value']);
				} else {
					$this->db->or_like($item, $_POST['columns']['4']['search']['value']);
				}
				if(count($this->column) -1 == $i)
					$this->db->group_end();
			}
